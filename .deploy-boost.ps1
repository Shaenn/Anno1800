# Dossier racine contenant Boost
$root = "$env:USERPROFILE\Documents\Anno 1800\mods"

# Dossier Boost
$boostPath = Join-Path $root "Boost"

# Récupérer tous les dossiers finaux
$finalDirs = Get-ChildItem -Path $boostPath -Recurse -Directory | Where-Object {
    # On considère un dossier "final" quand il est 3 niveaux sous Boost
    ($_.Parent.Parent.Parent.FullName -eq $boostPath) -and ($_.Parent.Parent -ne $null)
}

foreach ($dir in $finalDirs) {
    # Extraction des infos de hiérarchie
    $boost    = $dir.Parent.Parent.Parent.Name  # "Boost"
    ...
    $newName = "{0} - {1} - {2} - {3}" -f $boost, $world, $type, $dir.Name

    # Chemin de destination (à côté de Boost)
